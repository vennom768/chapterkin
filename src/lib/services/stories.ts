...
export async function createStoryForUser(userId: string, input: GenerateStoryInput) {
  try {
    await assertCanGenerateStory(userId);
  } catch (error) {
    return {
      ok: false as const,
  ...
  const parsed = generateStorySchema.parse({
    ...input,
  ...
  const profile = await getChildWithStoryCast(userId, parsed.childId);
  if (!profile) {
